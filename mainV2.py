...
class DroneAgent:
    ...
    # Get key that will be used to encrypt
    ...
    # Get key as a string to be sent over socket connection
    def get_str_key(self):
        password = b"password"
        salt = b"somesalt"
        backend = default_backend()
        kdf = PBKDF2HMAC(
                ...
                try:
                    ...
                    try:
                        if check_dist(self.state, client["state"]) <= CONNECTION_LIMIT:
                            ...
                            print(f"[CONNECTION] Connection made at {client['ip']}:{client['port']}")

                            
                            # Generate a random number as an identifier
                            identifier = secrets.token_hex(16)

                            # Join the drones' key with the identifier, separated by comma
                            joint_msg = self.str_key + ',' + identifier

                            # Encrypt
                            encrypted = self.key.encrypt(joint_msg.encode())

                            # Send the final message with encrypted [key, identifier]
                            client["conn"].send(encrypted)
                            print("Sending: ", joint_msg)
            ...
            while True:
                try:
                    ...
                    try:
                        ...
                        print("Received ", decrypted.decode("utf-8"))

                        # Check if the data contains the key
                        if self.str_key in decrypted.decode("utf-8"):
                            print("Found the key!")
                            # Identifier is sent as [key, identifier], so separate the identifier out
                            identifier = ''.join(decrypted.decode("utf-8").split(',')[1:])
                            print("The identifier is:", identifier)
                            continue
                    except:
                        print("Data not decryptable")
